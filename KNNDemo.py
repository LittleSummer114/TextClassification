...
def tocsv(data,vocab_word_file,name):
    ...
    temp_vocab_word = DataFrame(temp,columns=['vocab','df'])
...
def getVocab(params):
    ...
    vocab_word = read_csv(vocab_word_file, encoding='utf-8')['vocab'].tolist()
    vocab_char = read_csv(vocab_char_file, encoding='utf-8')['vocab'].tolist()
...
#使用全部数据预训练好所有的 char2vec + word2vec
def getPretrainedWord2vecs():
    conn = pymysql.connect(host='localhost', user='root', passwd='123456', db='data', charset='utf8')
    cursor1 = conn.cursor()
    cursor1.execute('select * from records')
    raw_data = cursor1.fetchall()
    cursor1.close()
    phenomenons = []
    corrects = []
    pretents = []
    ...
    for data in raw_data:
        if(count>100):
            break
        phenomenon = SplitSentence(data[2], stopwords, vocab)
        phenomenons.append(phenomenon)
        print(phenomenons)
        correct = SplitSentence(data[3], stopwords, vocab)
        corrects.append(correct)
        reason = SplitSentence(data[4], stopwords, vocab)
        reasons.append(reason)
        pretent = SplitSentence(data[5], stopwords, vocab)
        pretents.append(pretent)
        alls.append(phenomenon)
        alls.append(correct)
        alls.append(reason)
        alls.append(pretent)

    print(phenomenons)
    word2vec = Word2Vec(phenomenons, size=300, window=5, min_count=1)
    model = word2vec.wv
    model.save_word2vec_format('phenomenons_word2vecss.txt', binary=False)

    word2vec = Word2Vec(corrects, size=300, window=5, min_count=1)
    model = word2vec.wv
    model.save_word2vec_format('corrects_word2vecss.txt', binary=False)

    word2vec = Word2Vec(reasons, size=300, window=5, min_count=1)
    model = word2vec.wv
    model.save_word2vec_format('reasons_word2vecss.txt', binary=False)

    word2vec = Word2Vec(pretents, size=300, window=5, min_count=1)
    model = word2vec.wv
    model.save_word2vec_format('pretents_word2vecss.txt', binary=False)

    word2vec = Word2Vec(alls, size=300, window=5, min_count=1)
    model = word2vec.wv
    model.save_word2vec_format('alls_word2vecss.txt', binary=False)

if __name__=='__main__':
    getPretrainedWord2vecs()
